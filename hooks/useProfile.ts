...
export function useGetIdProfile(id: number) {
  return useQuery({
    queryFn: () => getIdProfile(id),
    queryKey: ["profile", id],
  });
}
...
export function useGetUserPostsHistory(id: number) {
  return useInfiniteQuery({
    queryKey: ["profile", id, "posts"],
    queryFn: ({ pageParam }) => getUserPostsHistory({ pageParam, id }),
    getNextPageParam: (lastPage) => {
      const nextPage = lastPage.page + 1;
      return lastPage.last ? undefined : nextPage;
    },
