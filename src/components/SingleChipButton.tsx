...
function SingleChipButton({ children, className, ...props }: ISingleChipButtonProps) {
  return (
    <RadioGroupPrimitive.Item
      ...
      className={cn(
        "w-fit h-16 px-8 py-5 text-body-1 font-semibold bg-gray-white hover:bg-gray-100 text-gray-600 cursor-pointer border border-gray-200 rounded-base data-[state=checked]:bg-main-primary data-[state=checked]:border-main-primary data-[state=checked]:text-gray-black disabled:border-gray-200 disabled:bg-gray-200 disabled:text-gray-400",
        className
      )}
      {...props}
    >
