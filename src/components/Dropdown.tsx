...
const Dropdown = ({ id, placeholder, label, items, ...props }: IDropdownProps) => {
  return (
        ...
        <SelectContent>
          <ScrollArea className="max-h-[232px]">
            {items.map((item) => (
              <SelectItem
                key={item.value}
                className="h-[58px] p-4 text-body-1 text-gray-400 focus:bg-gray-100 focus:text-gray-700 focus:font-semibold data-[state=checked]:bg-gray-100 data-[state=checked]:text-base data-[state=checked]:font-semibold data-[state=checked]:text-gray-700"
                value={item.value}
              >
                {item.title}
              </SelectItem>
